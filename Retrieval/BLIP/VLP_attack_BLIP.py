	...
	def run(self, image, aug_images, neg_image, text, aug_texts, neg_text, embeddings, ref_embeddings, max_length=30, args={}):
		...
		with torch.no_grad():
			for k, aug_sentences in enumerate(aug_texts):
				text_input = self.tokenizer(aug_sentences, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(device) 
				...
				aug_text_features.append(text_embed)
		...
		with torch.no_grad():
			...
			neg_text_tokens = self.tokenizer(neg_text, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(device) 
			neg_text_logit = self.model.text_encoder(neg_text_tokens.input_ids, attention_mask = neg_text_tokens.attention_mask, mode='text')  
			neg_text_features = F.normalize(self.model.text_proj(neg_text_logit.last_hidden_state[:,0,:]), dim=-1).unsqueeze(1)
		...
		with torch.no_grad():
			...
			text_tokens = self.tokenizer(text, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(device) 
			origin_text_features = self.model.text_encoder(text_tokens.input_ids, attention_mask = text_tokens.attention_mask, mode='text')  
			origin_text_features = F.normalize(self.model.text_proj(origin_text_features.last_hidden_state[:,0,:]), dim=-1)

			origin_image_features = self.model.visual_encoder(image.to(device))   
			origin_image_features = self.model.vision_proj(origin_image_features[:,0,:])            
			origin_image_features = F.normalize(origin_image_features, dim=-1)  
		...
		with torch.no_grad():
			for b in range(image.size(0)):
				...
				aug_image_embed = self.inference_image_2(aug_images[b, :])
				aug_image_feat = self.model.vision_proj(aug_image_embed[:,0,:])            
				image_features_aug.append(F.normalize(aug_image_feat, dim=-1).unsqueeze(0).to('cuda:0') ) 
		...
		neg_image_embed = self.inference_image_2(neg_image)
		neg_image_feat = self.model.vision_proj(neg_image_embed[:,0,:])            
		neg_image_features = F.normalize(neg_image_feat, dim=-1).unsqueeze(1).to('cuda:0').type(self.model.visual_encoder.pos_embed.dtype)
		...
		for i in range(args.num_iters):
			...
			for _ in torch.arange(self.m):
				...
				image_embed = self.inference_image_1(self.image_normalize(self.input_diversity(image)))
				image_feat = self.model.vision_proj(image_embed[:,0,:])            
				image_features = F.normalize(image_feat, dim=-1).to('cuda:0')
