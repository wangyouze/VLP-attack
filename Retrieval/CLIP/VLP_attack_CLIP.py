    ...
    def run(self, image, aug_images, neg_image, text, aug_texts, neg_text, embeddings, ref_embeddings, max_length=30, args={}):
        ...
        with torch.no_grad():
            for k, aug_sentences in enumerate(aug_texts):
                aug_text_features.append(self.model.inference(image[k,:].repeat(len(aug_sentences), 1, 1, 1).to(device), aug_sentences)['text_feat'])
            text_features_aug = torch.stack(aug_text_features, dim=0)



        with torch.no_grad():
            neg_text_logit = self.model.inference(neg_image.to(device), neg_text)
            neg_text_features = neg_text_logit['text_feat'].unsqueeze(1)
        with torch.no_grad():
            clean_logit = self.model.inference(image.to(device), text)
        ...
        with torch.no_grad():
            for b in range(image.size(0)):
                image_features_aug.append(self.inference_image_2(aug_images[b, :]).unsqueeze(0).to('cuda:0'))
        image_features_aug = torch.cat(image_features_aug, dim=0).type(self.model.visual.conv1.weight.dtype)
        neg_image_features = self.inference_image_2(neg_image).unsqueeze(1).to('cuda:0').type(self.model.visual.conv1.weight.dtype)

        text_features_all = torch.cat([ origin_text_features.unsqueeze(1),
                                        text_features_aug],
                                    dim=1).type(self.model.visual.conv1.weight.dtype)
        ...
        for i in range(args.num_iters):
            ...
            for _ in torch.arange(self.m):
                ...
                image_features = self.inference_image_1(self.image_normalize(self.input_diversity(image))).to('cuda:0')
                ...
                adv_text_features = adv_words_features.mean(1).unsqueeze(1).type(self.model.visual.conv1.weight.dtype)
